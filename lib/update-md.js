...
// Mandatory Reporting section to append if missing
const REPORTING_SECTION = `
## Mandatory Reporting / 필수 보고

Report to orchestrator on EVERY task completion. No exceptions.

\`\`\`
# Internal (aterm)
aterm inject orchestrator "REPORT: {modified files} | {change summary} | {build result} | {remaining issues}"
# External (telepty)
telepty inject --from $TELEPTY_SESSION_ID orchestrator "REPORT: ..."
\`\`\`

- NEVER idle or exit without reporting. / 보고 없이 대기/종료 금지.
- Include LESSONS in every report. / 모든 보고에 교훈 포함.
- 3 consecutive failures → report STUCK with full error. / 3회 연속 실패 → STUCK 보고.
- Evidence only — no "should work" or "probably fixed". / 증거만 — "아마 됐을 것" 금지.
`;
...
function updateMd({ projectPath, all = false, dryRun = false } = {}) {
  ...
  for (const dir of projectDirs) {
    ...
    for (const mdFile of MD_FILES) {
      ...
      // 3. Append Mandatory Reporting section if missing
      if (!content.includes("Mandatory Reporting") && !content.includes("필수 보고")) {
        content += REPORTING_SECTION;
        changes.push("added Mandatory Reporting section");
        changed = true;
      }
