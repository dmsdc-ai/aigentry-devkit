...
function getAtPath(root, parts) {
  ...
  for (const part of parts) {
    ...
    current = current[isIndex(part) ? Number(part) : part];
  }
  return current;
}

function ensureAtPath(root, parts) {
  let current = root;
  for (let i = 0; i < parts.length; i += 1) {
    ...
    const desired = isIndex(nextPart) ? [] : {};
...
const jsonNamedKey = {
  detect(filePath, keyPath, identifierPredicate) {
    if (!fs.existsSync(filePath)) return { present: false, entryIndex: null, entry: null };
    const { value } = parseJsonFile(filePath);
    const target = getAtPath(value, splitKeyPath(keyPath));
    if (!Array.isArray(target)) return { present: false, entryIndex: null, entry: null };
    const entryIndex = target.findIndex(identifierPredicate);
    if (entryIndex === -1) return { present: false, entryIndex: null, entry: null };
    return { present: true, entryIndex, entry: target[entryIndex] };
  },

  upsert(filePath, keyPath, identifierPredicate, newEntry, opts = {}) {
    const parsed = parseJsonFile(filePath);
    const target = ensureAtPath(parsed.value, splitKeyPath(keyPath));
    const entryIndex = target.findIndex(identifierPredicate);
    let action = "appended";
    if (entryIndex === -1) {
      target.push(newEntry);
    } else if (deepEqual(target[entryIndex], newEntry)) {
      return changedResult(false, "noop", null);
    } else {
      target[entryIndex] = newEntry;
      action = "replaced";
    }
    const nextText = `${JSON.stringify(parsed.value, null, parsed.indent)}\n`;
    const oldText = parsed.exists ? fs.readFileSync(filePath, "utf8") : "";
    if (nextText === oldText) return changedResult(false, "noop", null);
    const backupPath = backupFile(filePath, opts);
    atomicWriteFile(filePath, nextText, 0o644);
    return changedResult(true, action, backupPath);
  },

  remove(filePath, keyPath, identifierPredicate, opts = {}) {
    if (!fs.existsSync(filePath)) return changedResult(false, "noop", null);
    const parsed = parseJsonFile(filePath);
    const target = getAtPath(parsed.value, splitKeyPath(keyPath));
    if (!Array.isArray(target)) return changedResult(false, "noop", null);
    const entryIndex = target.findIndex(identifierPredicate);
    if (entryIndex === -1) return changedResult(false, "noop", null);
    target.splice(entryIndex, 1);
    const nextText = `${JSON.stringify(parsed.value, null, parsed.indent)}\n`;
