...
function renderBlock() {
  const template = fs.readFileSync(templatePath, "utf8").replace(/\{\{DEVKIT_VERSION\}\}/g, devkitVersion());
  const withZero = template.replace(/\{\{BLOCK_SHA256\}\}/g, "0".repeat(64));
  const digest = sha256(withZero);
  return template.replace(/\{\{BLOCK_SHA256\}\}/g, digest);
}
