...
function workspaceInit({ cli, cwd, orchestratorSessionId, autoReportErrors = true }) {
  ...
  if (!fs.existsSync(agentsDest)) {
    ...
    // Replace SAWP build/test commands based on detected project type
        ...
        PostToolUse: [
          {
            ...
            hooks: [
              {
                type: "command",
                command: `_ORCH="\${ATERM_ORCHESTRATOR_SESSION:-${orchestratorSessionId}}"; if [ "$CLAUDE_TOOL_EXIT_CODE" != "0" ] && [ -n "$TELEPTY_SESSION_ID" ] && [ -n "$_ORCH" ]; then telepty inject --from "$TELEPTY_SESSION_ID" "$_ORCH" "[BUILD ERROR] session: $TELEPTY_SESSION_ID | exit_code: $CLAUDE_TOOL_EXIT_CODE" 2>/dev/null; fi`,
              },
            ],
          },
        ],
